...
if [ ! -d "/data/data/com.termux" ]; then
    error "请在Termux环境中运行本脚本！"
fi

# 主流程
main() {
title "PURPLE" "Phira多人联机一键安装脚本"
info "CYAN" "即将自动安装依赖并构建服务，过程中请保持网络畅通~"
echo
# 添加作者信息
info "GREEN" "脚本作者: 日落-ya"
echo
# 等待三秒
info "YELLOW" "3秒后继续执行..."
sleep 3
...
# phira-mp启动提示
echo -e "'"${CYAN}"'======================================================='"${RESET}"'"
echo -e "'"${BOLD}${GREEN}"'Phira多人联机启动器'"${RESET}"'"
echo -e "'"${YELLOW}"'请按 [回车] 启动服务（按Ctrl+C退出）'"${RESET}"'"
echo -e "'"${CYAN}"'======================================================='"${RESET}"'"
read -r  # 等待用户回车
echo -e "'"${GREEN}"'正在启动服务...(≧▽≦)'"${RESET}"'"
if [ -d "phira-mp" ]; then
    cd phira-mp || { echo -e "'"${RED}"'无法进入phira-mp目录'"${RESET}"'"; exit 1; }
    RUST_LOG=info target/release/phira-mp-server
else
    echo -e "'"${RED}"'未找到phira-mp目录，请重新运行安装脚本'"${RESET}"'"
fi
'
